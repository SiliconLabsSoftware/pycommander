...
class TestNvm3(unittest.TestCase):
  def test_nvm3_delete_command(self):
    ...
    expected = [
      ...
      "--key", "key1", "--all", "--address", "0x08000000", "--range", "0x00000000:0x00002000", "--outfile", "out.nvm3",
      "--json",
    ]
  ...
  def test_nvm3_deletedevice_command(self):
    ...
    commander.nvm3.deletedevice(object_keys=["k1"], delete_all=True, range=(0x0, 0x1000))
    self.assertEqual(len(commander._runner.logged_commands), 1)
    expected = [
      "mock", "nvm3", "deletedevice",
      "--serialno", "123456789", "--key", "k1", "--all", "--range", "0x00000000:0x00001000",
      "--json",
    ]
    self.assertEqual(commander._runner.logged_commands[0], expected)
  ...
  def test_nvm3_initfile_command(self):
    commander = MockCommander()
    commander.nvm3.initfile("out.nvm3", 4096, "EFR32MG12", address=0x08000000, range=(0x0, 0x2000))
    self.assertEqual(len(commander._runner.logged_commands), 1)
    expected = [
      "mock", "nvm3", "initfile",
      "--outfile", "out.nvm3", "--size", "4096", "--device", "EFR32MG12", "--address", "0x08000000",
      "--range", "0x00000000:0x00002000",
      "--json",
    ]
    self.assertEqual(commander._runner.logged_commands[0], expected)
  ...
  def test_nvm3_parse_command(self):
    commander = MockCommander()
    commander.nvm3.parse("f.nvm3", object_keys=["k1"], nvm3file="out.nvm3", address=0x08000000, range=(0x0, 0x2000))
    self.assertEqual(len(commander._runner.logged_commands), 1)
    expected = [
  ...
  def test_nvm3_set_command(self):
    ...
    commander.nvm3.set(
      ...
      address=0x08000000,
      range=(0x0, 0x2000),
      objects=[(0x12, b"data1")],
      counters=[(0x56789, 123)],
      nvm3file="data.nvm3",
    )
    self.assertEqual(len(commander._runner.logged_commands), 1)
    expected = [
      ...
      "--outfile", "out.nvm3",
      "--address", "0x08000000", "--range", "0x00000000:0x00002000",
      "--object", "0x00012:6461746131", "--counter", "0x56789:123", "--nvm3file", "data.nvm3",
      "--json",
    ]
    self.assertEqual(commander._runner.logged_commands[0], expected)
